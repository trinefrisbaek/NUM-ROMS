program NUMmodeltest2
  ...
  real(dp), allocatable :: sumB(:,:,:),thebio(:,:,:,:)
  integer:: nBio,nDet,i
  integer :: kk
  ...
  allocate(sumB(NX,NY,NZ))
  allocate(thebio(nBio,NX,NY,NZ))
  print*, "Get netcdf data"
  ...
  SumB(:,:,:)=0.0d0
  thebio=u00(3:12,:,:,:)
  sumB=sum(thebio, dim=1)
  do z = 1,2
      do y = 1,2
          do x = 1, NX
  !            SumB(x,y,x)=SumB(x,y,z)
            !  write (*, *) z_w(x,y,z)-z_w(x,y,z-1)
         end do
      end do
  end do 
  ...
  print*, "start rate calculations"
  do x = 1, NX
      do y = 1, NY
          top_light=0.43d0*150.0d0*1025.0d0*3985.0d0
          mypar=top_light
          do z = NZ,1,-1
            Att_fac=(0.04d0+0.00003d0*MAX(0.0d0,sumB(x,y,z)))*  &
     &               (z_w(x,y,z+1)-z_w(x,y,z))
            ! print*, Att_fac
