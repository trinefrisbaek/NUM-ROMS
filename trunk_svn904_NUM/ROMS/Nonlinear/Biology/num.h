...
! TFHA: added variable definitions
      real(r8) :: tEnd, autobio_sum
      real(r8), dimension(10) :: Lratio
      integer:: ii
! TFHA: end
...
!
! TFHA: In NUM only detritus sinks
      idsink(1)=iSDeN
      idsink(2)=iLDeN
! TFHA: end
!
...
!
! TFHA: In NUM only detritus sinks
      Wbio(1)=wSDet(ng)               ! small Nitrogen-detritus
      Wbio(2)=wLDet(ng)               ! large Nitrogen-detritus
...
!  modify AttFac.
!
                autobio_sum=sum(Bio(i,k,iB001:iB010)*12.0107_r8)
                autobio_sum=MAX(0.0_r8,autobio_sum)
                Att=(AttSW(ng)+                                         &
     &               AttChl(ng)*autobio_sum+                            &
     &               AttFac)*                                           &
...
!-----------------------------------------------------------------------
!  TFHA: Prepare for and call NUM ROUTINE
!-----------------------------------------------------------------------
! TFH: This is where the num routine is called
               Bio(i,k,iNO3_)=Bio(i,k,iNO3_)*14.0067_r8 !NO3 mmol/m3 to ugN/l
               Bio(i,k,iDOC_)=Bio(i,k,iDOC_)*12.0107_r8
               Bio(i,k,iB001)=Bio(i,k,iB001)*12.0107_r8
               Bio(i,k,iB002)=Bio(i,k,iB002)*12.0107_r8
               Bio(i,k,iB003)=Bio(i,k,iB003)*12.0107_r8
               Bio(i,k,iB004)=Bio(i,k,iB004)*12.0107_r8
               Bio(i,k,iB005)=Bio(i,k,iB005)*12.0107_r8
               Bio(i,k,iB006)=Bio(i,k,iB006)*12.0107_r8
               Bio(i,k,iB007)=Bio(i,k,iB007)*12.0107_r8
               Bio(i,k,iB008)=Bio(i,k,iB008)*12.0107_r8
               Bio(i,k,iB009)=Bio(i,k,iB009)*12.0107_r8
               Bio(i,k,iB010)=Bio(i,k,iB010)*12.0107_r8

# if defined POM
! Detritus N to C
               Bio(i,k,iSDeN)=Bio(i,k,iSDeN)/16.0_r8*106.0_r8
               Bio(i,k,iLDeN)=Bio(i,k,iLDeN)/16.0_r8*106.0_r8
! Detritus units
               Bio(i,k,iSDeN)=Bio(i,k,iSDeN)*12.0107_r8
               Bio(i,k,iLDeN)=Bio(i,k,iLDeN)*12.0107_r8
# endif
               PAR=PAR*4.57_r8         !watts/m2 to micromole/m2/s
!************************* CALL NUM *******************************
               call simulateEuler(Bio(i,k,isalt+1:iLDeN), PAR, Bio(i,k,itemp), dtdays, dtdays,Lratio)
!************************* EXIT NUM *******************************
! Chlorophyll is based on the porportions between chl:carb and jL/L (Edwards et al, 2015)
               autobio_sum=sum(Bio(i,k,iB001:iB010)*Lratio)
               autobio_sum=MAX(0.0_r8,autobio_sum)
               Bio(i,k,iChlo)=(autobio_sum/53.9_r8)**(1.0_r8/0.64_r8) !ugChl/l = mgChl/m3

               PAR = PAR / 4.57_r8     !micromole/m2/s to watts/m2
               Bio(i,k,iNO3_)=Bio(i,k,iNO3_)/14.0067_r8
               Bio(i,k,iDOC_)=Bio(i,k,iDOC_)/12.0107_r8
               Bio(i,k,iB001)=Bio(i,k,iB001)/12.0107_r8
               Bio(i,k,iB002)=Bio(i,k,iB002)/12.0107_r8
               Bio(i,k,iB003)=Bio(i,k,iB003)/12.0107_r8
...
# if defined POM
! Detritus N to C
               Bio(i,k,iSDeN)=Bio(i,k,iSDeN)/106.0_r8*16.0_r8
               Bio(i,k,iLDeN)=Bio(i,k,iLDeN)/106.0_r8*16.0_r8
! Detritus units
               Bio(i,k,iSDeN)=Bio(i,k,iSDeN)/12.0107_r8
               Bio(i,k,iLDeN)=Bio(i,k,iLDeN)/12.0107_r8
# endif
! TFHA: This part is substituted by NUM
!!
...
!
          ...
          DO k=1,N(ng)
            DO i=Istr,Iend
              ...
              Bio(i,k,iDOC_)=Bio(i,k,iDOC_)+PhyCN(ng)*N_Flux_Remine
